...
class Snake:
    ...
    def move(self):
        ...
        if self.elements[0][0] > 780 or self.elements[0][0] < 20:
            screen.fill(black)
        ...
        elif self.elements[0][1] > 580 and self.elements[0][1] < 20:
            screen.fill(black)
            screen.blit(game_over, (220, 250))
            pygame.display.update()
            time.sleep(2)
    ...
    def collision_check(self):
        if snake.level >= 2:
            if self.elements[0][0] >= 300 and self.elements[0][0] <= 340 and self.elements[0][1] >= 0 and self.elements[0][1] <= 150:
                self.running = False
            elif self.elements[0][0] >= 150 and self.elements[0][0] <= 200 and self.elements[0][1] >= 300 and self.elements[0][1] <= HEIGHT:
                self.running=False
            return self.running
        else:
            return self.running
    ...
    def eat(self, foodx1, foody1): 
        ...
        #return False
        if self.score >= 15:
            pygame.draw.rect(screen, red, (a, b, 100, 100), 10)            
             
    #yellow food with bigger weight
    def eatBIG(self,bfoodx,bfoody):
    ...
    #blue food dissapear after 5 second
    def eatTime(self,tfoodx,tfoody): 
    ...
    def gen(self):
        ...
        if snake.level >= 2:
            if self.x >= 300 and self.x <= 340:
                self.y = random.randint(151, HEIGHT)
...
while snake.collision_check() and running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == wall_event:
            pygame.draw.rect(screen, red, (380, 300, 280, 320), 10)
